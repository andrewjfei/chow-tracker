const styles = {
  button: {
    base: 'px-4 py-2 border-2 rounded-lg text-base font-regular transition-colors hover:cursor-pointer focus:outline-none focus:border-orange-600',
    filled: {
      default:
...
const Card = ({ className, children, primary }) => {
  return (
    <div className={`${className} p-5 bg-white rounded-lg drop-shadow-md`}>
      {children}
    </div>
  );
};
